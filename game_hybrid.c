...
int main(int argc, char **argv)
{
    ...
    #pragma omp parallel num_threads(10) for private(our_current_row, my_current_column)
    ...
    #pragma omp parallel num_threads(10) for private(current_time_step,my_current_column)
            ...
            current_time_step++)
    {
        ...
        #pragma omp parallel num_threads(10) for private(our_current_row)
        for(our_current_row = 0; our_current_row <= OUR_NUMBER_OF_ROWS + 1;
                our_current_row++)
        {
...
#ifdef SHOW_RESULTS
        #pragma omp parallel num_threads(10) for private(current_rank)
        ...
        #pragma omp parallel num_threads(10) for private(our_current_row,my_current_column,my_neighbor_row,my_neighbor_column)
        for(our_current_row = 1; our_current_row <= OUR_NUMBER_OF_ROWS;
                our_current_row++)
        {
